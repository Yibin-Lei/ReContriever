...
def load_data(opt, tokenizer):
    ...
    for path in opt.train_data:
        ...
        if data is not None:
            if opt.crop_method == "normal":
                datasets[path] = Dataset_normal(data, opt.chunk_length, tokenizer, opt)
            if opt.crop_method == "4_pair":
                datasets[path] = Dataset_4_pair(data, opt.chunk_length, tokenizer, opt)
...
class Dataset_normal(torch.utils.data.Dataset):
...
class Dataset_4_pair(torch.utils.data.Dataset):
    """Monolingual dataset based on a list of paths"""
    ...
    def __getitem__(self, index):
        ...
        q_tokens = randomcrop_word(tokens, self.opt.ratio_min, self.opt.ratio_max)
        k1_tokens = randomcrop_word(tokens, self.opt.ratio_min, self.opt.ratio_max)
        k2_tokens = randomcrop_word(tokens, self.opt.ratio_min, self.opt.ratio_max)
        k3_tokens = randomcrop_word(tokens, self.opt.ratio_min, self.opt.ratio_max)
        k4_tokens = randomcrop_word(tokens, self.opt.ratio_min, self.opt.ratio_max)

        q_tokens = apply_augmentation(q_tokens, self.opt)
        q_tokens = add_bos_eos(q_tokens, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)
        k1_tokens = apply_augmentation(k1_tokens, self.opt)
        k1_tokens = add_bos_eos(k1_tokens, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)
        k2_tokens = apply_augmentation(k2_tokens, self.opt)
        k2_tokens = add_bos_eos(k2_tokens, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)
        k3_tokens = apply_augmentation(k3_tokens, self.opt)
        k3_tokens = add_bos_eos(k3_tokens, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)
        k4_tokens = apply_augmentation(k4_tokens, self.opt)
        k4_tokens = add_bos_eos(k4_tokens, self.tokenizer.bos_token_id, self.tokenizer.eos_token_id)

        return {"q_tokens": q_tokens, "k1_tokens": k1_tokens, "k2_tokens": k2_tokens, "k3_tokens": k3_tokens,
                "k4_tokens": k4_tokens}
...
class Collator_4_pair(object):
    ...
    def __call__(self, batch_examples):
        ...
        q_tokens, q_mask = build_mask(batch["q_tokens"])
        k1_tokens, k1_mask = build_mask(batch["k1_tokens"])
        k2_tokens, k2_mask = build_mask(batch["k2_tokens"])
        k3_tokens, k3_mask = build_mask(batch["k3_tokens"])
        k4_tokens, k4_mask = build_mask(batch["k4_tokens"])


        batch["q_tokens"] = q_tokens
        batch["q_mask"] = q_mask
        batch["k1_tokens"] = k1_tokens
        batch["k1_mask"] = k1_mask
        batch["k2_tokens"] = k2_tokens
        batch["k2_mask"] = k2_mask
        batch["k3_tokens"] = k3_tokens
        batch["k3_mask"] = k3_mask
        batch["k4_tokens"] = k4_tokens
        batch["k4_mask"] = k4_mask
