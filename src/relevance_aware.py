...
class RelevanceAware(nn.Module):
    def __init__(self, opt):
        super(RelevanceAware, self).__init__()
    ...
    def forward(self, q_tokens, q_mask, k1_tokens, k1_mask, k2_tokens, k2_mask, k3_tokens, k3_mask, k4_tokens, k4_mask, stats_prefix="", iter_stats={}, **kwargs):
        bsz = q_tokens.size(0)

        q = self.encoder_q(input_ids=q_tokens, attention_mask=q_mask, normalize=self.norm_query)
        q = torch.cat([q, q, q, q], dim=0)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k_tokens = torch.cat([k1_tokens, k2_tokens, k3_tokens, k4_tokens], dim=0)
            k_mask = torch.cat([k1_mask, k2_mask, k3_mask, k4_mask], dim=0)
            k = self.encoder_k(input_ids=k_tokens, attention_mask=k_mask, normalize=self.norm_doc)

            # estimate the relevance between all positive pairs
            relevance = torch.einsum("nc,nc->n", [q, k])
            relevance = relevance.view(-1, bsz)
            relevance = relevance / sim.sum(dim=0, keepdim=True) # normalize the weights among all the 4 positive pairs originating from the same document
            relevance = relevance.view(4 * bsz)

        logits = self._compute_logits(q, k) / self.temperature

        # labels: positive key indicators
        labels = torch.zeros(bsz*4, dtype=torch.long).cuda()

        loss = self.weighted_cross_entropy(logits, relevance)
    ...
    def weighted_cross_entropy(self, logits, weight):
        return (- torch.nn.functional.log_softmax(logits, dim=1)[:, 0] * weight).mean()
